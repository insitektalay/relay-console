  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="delete-marketplace-connection-title"
        className="w-full max-w-md rounded-[6px] border border-[color-mix(in_srgb,var(--claw-border)_54%,transparent)] bg-[var(--claw-bg-surface)] p-4 shadow-2xl"
      >
        <div
          id="delete-marketplace-connection-title"
          className="text-base font-semibold text-[var(--claw-text-primary)]"
        >
          Delete connection?
        </div>
        <p className="mt-2 text-sm leading-6 text-[var(--claw-text-secondary)]">
          Relay will delete {connectionName} and remove {appName} from all
          assigned agents.
        </p>
        <p className="mt-2 text-xs leading-5 text-[var(--claw-text-muted)]">
          Revoke the credential or authorization in the provider dashboard if
          you do not want it to remain valid.
        </p>
        <div className="mt-4 flex justify-end gap-2">
          <Button
            type="button"
