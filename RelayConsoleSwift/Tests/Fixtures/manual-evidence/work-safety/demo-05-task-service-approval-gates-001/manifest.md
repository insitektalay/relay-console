# Manual Evidence Manifest - Demo 5 Task Service Approval Gates

id: `fix-manual-work-safety-demo-05-task-service-approval-gates-001`
...
gapOrDecisionIds: `ITC-0040`, `APPROVALS-NAV-EXCLUDED-001`, `CONTROLLED-WRITES-LATER-001`, `LOCAL-FILE-ACCESS-EXCLUDED-001`
...
sourceBaseline: `itc-0039-task-service-scheduling-targeting-approval-gates-packet-dry-run.md`, `ServiceTests.swift`, `WorkSafetyTaskService.swift`

files:

- `manual-evidence/work-safety/demo-05-task-service-approval-gates-001/manifest.md`
...
surface: `Demo 5 task service approval-gate review placeholder`

stateKind: `pending`

reasonCode: `manual-demo-05-task-service-approval-gates-pending`
...
missingPrerequisites: `Manual reviewer notes, standalone Approvals queue/detail, permission-policy parity, approval resolver UI, controlled writes, native file access, and release proof remain later evidence.`

currentState: `Automated tests verify task target resolution, schedule updates, approval-required dispatch blocking across relaunch, approved retained dispatch, cancellation, retry, and no runtime action rows. Demo 5 manual observations are not yet claimed.`

notParityStatement: `This planned manifest is not completed Demo 5 proof, standalone Approvals proof, executable task dispatch proof, controlled-write proof, native file access proof, provider write proof, source-host/local app proof, or release readiness.`
...
releaseImpact: `Keeps ITC-0039 manual safety residual explicit while allowing automated service evidence to close.`

determinism: `The placeholder uses fixed ids and contains no environment-specific values.`

noFakeProductSeed: `No product-visible task, approval, action-run, source-host, scheduled-message, audit, or runtime output rows are seeded.`
...
failureHandling: `If this planned manifest is cited as verified manual Demo 5 proof, standalone Approvals proof, controlled-write proof, or executable work proof, ITC-0039 closeout must be downgraded to partial-proof or no-go.`
