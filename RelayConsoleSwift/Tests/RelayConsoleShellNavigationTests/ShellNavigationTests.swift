...
struct RelayConsoleShellNavigationTests {
    ...
    private static func testSwiftUISourceUsesGuardedShellControls() throws {
        ...
        let chatScreen = try readPackageFile("Sources/RelayConsoleApp/Features/Chats/ChatScreen.swift")
        let agentDetails = try readPackageFile("Sources/RelayConsoleApp/Features/Agents/AgentDetailViews.swift")
        let agentKnowledge = try readPackageFile("Sources/RelayConsoleApp/AgentKnowledgeViews.swift")
        let applications = try readPackageFile("Sources/RelayConsoleApp/Features/Applications/ApplicationCatalogViews.swift")
        ...
        try expect(views.contains("@State private var navigationPanelsVisible = true"), "ContentView should own navigation panel visibility")
        try expect(views.contains("if navigationPanelsVisible"), "ContentView should conditionally render both navigation panels")
        try expect(views.contains("NavigationPanelsToggle(isVisible: $navigationPanelsVisible)"), "ContentView should keep the navigation toggle outside the collapsible panels")
        try expect(views.contains("MainStage(navigationPanelsVisible: $navigationPanelsVisible)"), "ContentView should pass navigation visibility into the main stage")
        try expect(chatScreen.contains("@Binding var navigationPanelsVisible: Bool"), "ChatScreen should observe navigation visibility")
        try expect(chatScreen.contains(".padding(.leading, navigationPanelsVisible ? 0 : 44)"), "Collapsed chat header should reserve the toggle slot before its avatar")
        try expect(views.contains("AgentsScreen(navigationPanelsVisible: navigationPanelsVisible)"), "MainStage should pass navigation visibility to Agents")
        try expect(agentDetails.contains("navigationPanelsVisible: navigationPanelsVisible"), "Agents should pass navigation visibility to knowledge headers")
        try expect(agentKnowledge.contains(".padding(.leading, navigationPanelsVisible ? 0 : 44)"), "Collapsed agent header should reserve the toggle slot")
        try expect(views.contains("ApplicationsScreen(navigationPanelsVisible: navigationPanelsVisible)"), "MainStage should pass navigation visibility to Applications")
        try expect(applications.contains("ApplicationsCatalogPanel(navigationPanelsVisible: navigationPanelsVisible)"), "Applications should pass navigation visibility to its catalog header")
        try expect(applications.contains(".padding(.leading, navigationPanelsVisible ? 0 : 44)"), "Collapsed applications header should reserve the toggle slot")
        try expect(views.contains("HeaderIconControl(symbolName: \"sidebar.left\")"), "Navigation toggle should use the same control geometry as chat header actions")
        try expect(views.contains("RCChromeMetrics.topReservedHeight - ChatHeaderControlStyle.height"), "Navigation toggle should align to the chat header control baseline")
        try expect(views.contains("Hide navigation panels"), "Navigation toggle should expose its collapse action")
        try expect(views.contains("Show navigation panels"), "Navigation toggle should expose its restore action")
