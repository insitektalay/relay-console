...
struct ApplicationsExaSearchDetailPanel: View {
  let app: MarketplaceCatalogApp

  var body: some View {
    VStack(spacing: 14) {
      ApplicationsExaHeroCard(app: app)
      ApplicationsExaAgentsCard(app: app)
      ApplicationsExaConnectionsCard(app: app)
      ApplicationsInfoCardsLayout {
        ApplicationsExaInfoCard(
          icon: "sparkles",
          title: "Capabilities",
          items: app.capabilities,
          linkTitle: "Learn more about Exa Search",
        ...
        ApplicationsExaInfoCard(
          ...
          items: [
            "Run source-backed web searches from their local runtime profile",
            "Fetch page contents and highlights for selected URLs",
            "Use only the selected Exa key assigned through this page",
          ],
          linkTitle: nil,
          linkURL: nil
        )
...
struct ApplicationsExaSearchConnectionPanel: View {
  let app: MarketplaceCatalogApp
...
struct ApplicationsExaConnectionsCard: View {
  @EnvironmentObject var model: AppViewModel
  let app: MarketplaceCatalogApp
  ...
  private var canAdd: Bool {
    model.exaAPIConnectionNameDraft.nilIfEmpty != nil
      && model.exaAPIKeyDraft.nilIfEmpty != nil
      && model.busy != "add-exa-api-connection"
  }
    ...
    ApplicationsExaPanel {
      HStack(alignment: .top, spacing: 14) {
        ApplicationsExaSectionHeading(
          ...
          subtitle: "Add, test, and manage your Exa API keys.")
        Spacer()
        ApplicationsExaInfoPill(text: "Selecting a connection updates the agent assignments above.")
      }
      ...
      if let status = model.exaConnectionStatus?.nilIfEmpty {
        HStack(spacing: 8) {
          Image(
          ...
          .foregroundStyle(
            status.localizedCaseInsensitiveContains("connected")
              ...
              || status.localizedCaseInsensitiveContains("verified")
              ? RCTheme.accentGreen : RCTheme.accentAmber)
          Text(status)
            .font(.system(size: 12, weight: .semibold))
            .foregroundStyle(RCTheme.muted)
    ...
    VStack(alignment: .leading, spacing: 12) {
      ApplicationsConnectionFormGrid {
        ApplicationsExaInput(
          label: "Connection name", placeholder: "Research Key A",
          text: $model.exaAPIConnectionNameDraft, secure: false)
        ApplicationsExaInput(
          label: "Exa API key", placeholder: "exa_••••••••••••••••••••",
          text: $model.exaAPIKeyDraft, secure: true)
      }
      Button {
        model.addExaAPIConnection(for: app)
      } label: {
        if model.busy == "add-exa-api-connection" {
          HStack(spacing: 7) {
            ProgressView()
              .controlSize(.small)
              .scaleEffect(0.75)
      ...
      .disabled(
        !canAdd || model.providerConnectionSnapshot?.readOnly == true
          || app.availability != .available)
      HStack(spacing: 7) {
        Image(systemName: "lock")
        Text("Keys are stored securely in your local Keychain.")
      }
  ...
  private var connectionTable: some View {
    VStack(spacing: 0) {
      ApplicationsExaConnectionHeader()
      if connections.isEmpty {
        EmptyMiniLight(
          title: "No API connections", body: "Add a named Exa API key to begin assigning agents."
        )
        .padding(.vertical, 22)
      } else {
        ForEach(connections) { connection in
          ApplicationsExaConnectionRow(
            app: app,
            connection: connection,
      ...
      HStack(spacing: 8) {
        Image(systemName: "lightbulb")
          .foregroundStyle(RCTheme.muted)
        Text("Use the test action to verify a key. Only valid keys can be set as active.")
          .font(.system(size: 12, weight: .semibold))
          .foregroundStyle(RCTheme.muted)
        Spacer()
      }
...
struct ApplicationsExaConnectionHeader: View {
  var body: some View {
    HStack(spacing: 12) {
      Text("")
        .frame(width: 28)
