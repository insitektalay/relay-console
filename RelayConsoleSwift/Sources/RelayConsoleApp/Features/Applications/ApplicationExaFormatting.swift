...
struct ApplicationsExaConnectionRow: View {
  ...
  private var isTesting: Bool {
    model.busy == "test-exa-api-connection-\(connection.id)"
  }

  private var isDeleting: Bool {
    model.busy == "delete-exa-api-connection-\(connection.id)"
  }
  ...
  var body: some View {
    HStack(spacing: 12) {
      ...
      .help("Select \(exaConnectionName(connection))")
      .accessibilityLabel("Select \(exaConnectionName(connection))")
      ...
      HStack(spacing: 7) {
        Image(
          systemName: exaConnectionIsValid(connection)
            ? "checkmark.circle.fill" : "exclamationmark.triangle.fill"
        )
        .foregroundStyle(
          exaConnectionIsValid(connection) ? RCTheme.accentGreen : RCTheme.accentAmber)
        Text(exaConnectionStatusText(connection))
          .font(.system(size: 12, weight: .semibold))
      }
      .frame(width: 120, alignment: .leading)

      Text(exaLastTestedText(connection))
        .font(.system(size: 12, weight: .semibold))
        .foregroundStyle(RCTheme.muted)
        .frame(width: 120, alignment: .leading)
      ...
      HStack(spacing: 8) {
        Button {
          model.testExaAPIConnection(connection, for: app)
        } label: {
          if isTesting {
            ProgressView()
              .controlSize(.small)
        ...
        .disabled(isTesting || isDeleting || model.providerConnectionSnapshot?.readOnly == true)
        .help("Test \(exaConnectionName(connection))")
        .accessibilityLabel("Test \(exaConnectionName(connection))")

        Button {
          model.deleteExaAPIConnection(connection, for: app)
        } label: {
          Image(systemName: "trash")
        }
        .buttonStyle(IconLightButtonStyle())
        .disabled(isTesting || isDeleting || model.providerConnectionSnapshot?.readOnly == true)
        .help("Delete \(exaConnectionName(connection))")
        .accessibilityLabel("Delete \(exaConnectionName(connection))")
      }
