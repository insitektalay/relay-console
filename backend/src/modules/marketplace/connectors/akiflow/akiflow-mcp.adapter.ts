...
export const AKIFLOW_READ_TOOLS = [
  "get-calendars",
  "get-events",
  "get-meeting-brief",
  "get-meeting-briefs",
...
export const AKIFLOW_MANAGE_TOOLS = [
  "add-event",
  "add-task",
...
export class AkiflowMcpError extends Error {
  constructor(
...
@Injectable()
export class AkiflowMcpAdapter {
  async health(accessToken: string) {
    return this.withSession(accessToken, async (session) => {
      ...
      for (const required of [...AKIFLOW_READ_TOOLS, ...AKIFLOW_MANAGE_TOOLS]) {
        if (!tools.some((tool) => tool.name === required)) {
          throw new AkiflowMcpError(
            "provider_validation_error",
            `Akiflow MCP did not expose ${required} with a valid object schema.`,
          );
        }
      }
      return {
        ...
        readToolCount: AKIFLOW_READ_TOOLS.length,
        manageToolCount: AKIFLOW_MANAGE_TOOLS.length,
      };
    });
  }
  ...
  callRead(accessToken: string, input: JsonObject) {
    return this.callAllowed(
      accessToken,
      input,
      new Set<string>(AKIFLOW_READ_TOOLS),
    );
  }
  callManage(accessToken: string, input: JsonObject) {
    return this.callAllowed(
      accessToken,
      input,
      new Set<string>(AKIFLOW_MANAGE_TOOLS),
    );
  }
  ...
  private async callAllowed(
    ...
    if (!allowed.has(toolName))
      throw new AkiflowMcpError(
        "policy_blocked",
        `Akiflow MCP tool ${toolName} is not allowed by this Relay action.`,
        403,
      );
    ...
    if (Buffer.byteLength(JSON.stringify(args)) > 1_000_000)
      throw new AkiflowMcpError(
        "provider_validation_error",
        "Akiflow MCP arguments exceed 1 MB.",
      );
    return this.withSession(accessToken, async (session) => {
      ...
      if (!tool)
        throw new AkiflowMcpError(
          "provider_validation_error",
          `Akiflow MCP did not expose ${toolName} with a valid object schema.`,
        );
  ...
  private async withSession<T>(
    ...
    if (!accessToken)
      throw new AkiflowMcpError(
        "credential_missing",
        "Akiflow OAuth access token is required.",
        401,
      );
    ...
    if (!this.object(initialized.capabilities).tools)
      throw new AkiflowMcpError(
        "provider_validation_error",
        "Akiflow MCP did not advertise tool support.",
      );
  ...
  private async rpc(
    ...
    if (Buffer.byteLength(raw) > 5_000_000)
      throw new AkiflowMcpError(
        "provider_validation_error",
        "Akiflow MCP response exceeds 5 MB.",
      );
    ...
    if (!payload)
      throw new AkiflowMcpError(
        ...
        "Akiflow MCP returned an empty response.",
        502,
      );
    if (payload.error)
      throw new AkiflowMcpError(
  ...
  private async request(
    ...
    try {
      const response = await safeConnectorFetch("https://mcp.akiflow.com/mcp", {
        method: "POST",
        headers: {
      ...
      if (response.status === 401)
        throw new AkiflowMcpError(
          "credential_missing",
          "Akiflow MCP rejected the OAuth token.",
          401,
        );
      if (response.status === 403)
        throw new AkiflowMcpError(
          "insufficient_scope",
          "Akiflow MCP denied this capability.",
          403,
        );
      if (response.status === 429)
        throw new AkiflowMcpError(
          "provider_rate_limited",
          "Akiflow MCP rate limit reached.",
          429,
        );
      if (!response.ok && response.status !== 202 && response.status !== 204)
        throw new AkiflowMcpError(
          response.status >= 500
            ? "provider_unavailable"
            : "provider_validation_error",
          `Akiflow MCP returned HTTP ${response.status}.`,
          response.status,
        );
      return response;
    } catch (error) {
      if (error instanceof AkiflowMcpError) throw error;
      throw new AkiflowMcpError(
        "provider_unavailable",
        "Akiflow MCP could not be reached.",
        502,
      );
    }
  }
  ...
  private rejectCredentialFields(value: unknown, depth = 0) {
    if (depth > 12)
      throw new AkiflowMcpError(
        "policy_blocked",
        "Akiflow MCP arguments are too deeply nested.",
        403,
      );
    if (Array.isArray(value)) {
      if (value.length > 500)
        throw new AkiflowMcpError(
          "provider_validation_error",
          "Akiflow MCP arrays exceed 500 items.",
        );
    ...
    if (entries.length > 500)
      throw new AkiflowMcpError(
        "provider_validation_error",
        "Akiflow MCP objects exceed 500 fields.",
      );
