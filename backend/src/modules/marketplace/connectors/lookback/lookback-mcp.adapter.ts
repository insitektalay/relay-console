...
const MUTATION =
  /(^|[_.-])(create|update|edit|delete|remove|publish|write|send|invite|assign|approve|archive|restore|upload|execute|run|set)([_.-]|$)/i;
const MUTATION_TEXT =
  /\b(create|update|edit|delete|remove|publish|write|send|invite|assign|approve|archive|restore|upload|execute|run|set up)\b/i;

export class LookbackMcpError extends Error {
  constructor(
...
@Injectable()
export class LookbackMcpAdapter {
  async health(accessToken: string) {
    return this.withSession(accessToken, async (session) => {
      ...
      if (!readable.length)
        throw new LookbackMcpError(
          "provider_validation_error",
          "Lookback MCP exposed no verifiably non-mutating tools.",
        );
      return {
  ...
  async callRead(accessToken: string, input: JsonObject) {
    ...
    if (MUTATION.test(toolName))
      throw new LookbackMcpError(
        "policy_blocked",
        `Lookback MCP tool ${toolName} appears mutating.`,
        403,
      );
    ...
    if (Buffer.byteLength(JSON.stringify(args)) > 500_000)
      throw new LookbackMcpError(
        "provider_validation_error",
        "Lookback MCP arguments exceed 500 KB.",
      );
    return this.withSession(accessToken, async (session) => {
      ...
      if (!tool || !this.isReadOnly(tool))
        throw new LookbackMcpError(
          "policy_blocked",
          `Lookback MCP tool ${toolName} is not a verified non-mutating tool.`,
          403,
        );
  ...
  private async withSession<T>(
    ...
    if (!accessToken)
      throw new LookbackMcpError(
        "credential_missing",
        "Lookback OAuth access token is required.",
        401,
      );
    ...
    if (!this.object(initialized.capabilities).tools)
      throw new LookbackMcpError(
        "provider_validation_error",
        "Lookback MCP did not advertise tool support.",
      );
  ...
  private async rpc(
    ...
    if (Buffer.byteLength(raw) > 3_000_000)
      throw new LookbackMcpError(
        "provider_validation_error",
        "Lookback MCP response exceeds 3 MB.",
      );
    ...
    if (!payload)
      throw new LookbackMcpError(
        ...
        "Lookback MCP returned an empty response.",
        502,
      );
    if (payload.error)
      throw new LookbackMcpError(
  ...
  private async request(
    ...
    try {
      const response = await safeConnectorFetch("https://mcp.lookback.io/mcp", {
        method: "POST",
        headers: {
      ...
      if (response.status === 401)
        throw new LookbackMcpError(
          "credential_missing",
          "Lookback MCP rejected the OAuth token.",
          401,
        );
      if (response.status === 403)
        throw new LookbackMcpError(
          "insufficient_scope",
          "Lookback MCP denied this capability.",
          403,
        );
      if (response.status === 429)
        throw new LookbackMcpError(
          "provider_rate_limited",
          "Lookback MCP rate limit reached.",
          429,
        );
      if (!response.ok && response.status !== 202 && response.status !== 204)
        throw new LookbackMcpError(
          response.status >= 500
            ? "provider_unavailable"
            : "provider_validation_error",
          `Lookback MCP returned HTTP ${response.status}.`,
          response.status,
        );
      return response;
    } catch (error) {
      if (error instanceof LookbackMcpError) throw error;
      throw new LookbackMcpError(
        "provider_unavailable",
        "Lookback MCP could not be reached.",
        502,
      );
    }
  }
  ...
  private rejectCredentialFields(value: unknown, depth = 0) {
    if (depth > 12)
      throw new LookbackMcpError(
        "policy_blocked",
        "Lookback MCP arguments are too deeply nested.",
        403,
      );
