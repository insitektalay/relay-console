...
@Injectable()
export class AhaApiAdapter {
  health(token: string, accountSubdomain: string) {
    return this.request(token, accountSubdomain, {
      method: "GET",
      path: "/api/v1/me",
    });
  }

  read(token: string, accountSubdomain: string, input: JsonObject) {
    return this.request(token, accountSubdomain, {
      method: "GET",
  ...
  manage(token: string, accountSubdomain: string, input: JsonObject) {
    return this.request(token, accountSubdomain, {
  ...
  async request(
    ...
    if (!token)
      throw new AhaApiError(
        "credential_missing",
        "Aha! access token is required.",
        401,
      );
    const subdomain = this.accountSubdomain(accountSubdomain);
    ...
    if (
      ...
      throw this.validation("Aha! method or REST v1 path is invalid.");
    }
    ...
    if (body && Buffer.byteLength(body) > 2_000_000)
      throw this.validation("Aha! request exceeds 2 MB.");

    const url = new URL(`https://${subdomain}.aha.io${input.path}`);
    ...
    if (Buffer.byteLength(raw) > 10_000_000)
      throw this.validation("Aha! response exceeds 10 MB.");
    ...
    if (!response.ok)
      throw new AhaApiError(
        ...
        this.message(data) ?? `Aha! returned HTTP ${response.status}.`,
        response.status,
      );
    return data;
  }

  private accountSubdomain(value: string) {
    ...
    if (
      ...
      throw this.validation("Aha! account binding is invalid.");
    return subdomain;
  }
  ...
  private appendQuery(params: URLSearchParams, value?: JsonObject) {
    ...
    if (Object.keys(value).length > 50)
      throw this.validation("Aha! query has too many fields.");
    for (const [key, item] of Object.entries(value)) {
      if (!/^[A-Za-z0-9_.\[\]-]{1,100}$/.test(key))
        throw this.validation("Aha! query field is invalid.");
      if (item == null || item === "") continue;
      const entries = Array.isArray(item) ? item : [item];
      if (entries.length > 100)
        throw this.validation("Aha! query array is too large.");
      for (const entry of entries) {
        if (!["string", "number", "boolean"].includes(typeof entry))
          throw this.validation("Aha! query value is invalid.");
  ...
  private rejectSecrets(value?: JsonObject) {
    const walk = (item: unknown, depth = 0) => {
      if (depth > 12)
        throw new AhaApiError(
          "policy_blocked",
          "Aha! request is too deeply nested.",
        );
  ...
  private validation(message: string) {
    return new AhaApiError("provider_validation_error", message);
  }
