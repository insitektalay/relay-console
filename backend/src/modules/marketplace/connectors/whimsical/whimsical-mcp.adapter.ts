...
export const WHIMSICAL_READ_TOOLS = [
  "search",
  "fetch",
  "file_tree",
  "list_workspaces",
...
export const WHIMSICAL_WRITE_TOOLS = [
  "create",
  "doc_create",
  "edit",
  "wireframe_edit",
...
export class WhimsicalMcpError extends Error {
  constructor(
...
@Injectable()
export class WhimsicalMcpAdapter {
  async health(accessToken: string) {
    return this.withSession(accessToken, async (session) => {
      ...
      for (const required of [...WHIMSICAL_READ_TOOLS, ...WHIMSICAL_WRITE_TOOLS]) {
        if (!tools.some((tool) => tool.name === required)) {
          throw new WhimsicalMcpError(
            "provider_validation_error",
            `Whimsical MCP did not expose the documented ${required} tool with a valid object schema.`,
          );
        }
      }
      return {
        ...
        readToolCount: WHIMSICAL_READ_TOOLS.length,
        writeToolCount: WHIMSICAL_WRITE_TOOLS.length,
      };
    });
  }

  callRead(accessToken: string, input: JsonObject) {
    return this.callAllowed(accessToken, input, new Set<string>(WHIMSICAL_READ_TOOLS));
  }

  callWrite(accessToken: string, input: JsonObject) {
    return this.callAllowed(accessToken, input, new Set<string>(WHIMSICAL_WRITE_TOOLS));
  }
  ...
  private async callAllowed(accessToken: string, input: JsonObject, allowed: Set<string>) {
    ...
    if (!allowed.has(toolName)) {
      throw new WhimsicalMcpError(
        "policy_blocked",
        `Whimsical MCP tool ${toolName} is not allowed by this Relay action.`,
        403,
      );
    }
    ...
    if (Buffer.byteLength(JSON.stringify(args)) > 1_000_000) {
      throw new WhimsicalMcpError(
        "provider_validation_error",
        "Whimsical MCP arguments exceed 1 MB.",
      );
    }
    return this.withSession(accessToken, async (session) => {
      ...
      if (!tool) {
        throw new WhimsicalMcpError(
          "provider_validation_error",
          `Whimsical MCP did not expose ${toolName} with a valid object schema.`,
        );
      }
  ...
  private async withSession<T>(accessToken: string, fn: (session: Session) => Promise<T>) {
    if (!accessToken) {
      throw new WhimsicalMcpError(
        "credential_missing",
        "Whimsical OAuth access token is required.",
        401,
      );
    }
    ...
    if (!this.object(initialized.capabilities).tools) {
      throw new WhimsicalMcpError(
        "provider_validation_error",
        "Whimsical MCP did not advertise tool support.",
      );
    }
  ...
  private async rpc(accessToken: string, session: Session, method: string, params: JsonObject) {
    ...
    if (Buffer.byteLength(raw) > 5_000_000) {
      throw new WhimsicalMcpError(
        "provider_validation_error",
        "Whimsical MCP response exceeds 5 MB.",
      );
    }
    ...
    if (!payload) {
      throw new WhimsicalMcpError(
        ...
        "Whimsical MCP returned an empty response.",
        502,
      );
    }
    if (payload.error) {
      throw new WhimsicalMcpError(
        "provider_validation_error",
        this.string(this.object(payload.error).message) ?? "Whimsical MCP request failed.",
      );
    }
  ...
  private async request(accessToken: string, session: Session, body: JsonObject) {
    try {
      const response = await safeConnectorFetch("https://mcp.whimsical.com/mcp", {
        method: "POST",
        headers: {
      ...
      if (response.status === 401) {
        throw new WhimsicalMcpError(
          "credential_missing",
          "Whimsical MCP rejected the OAuth token.",
          401,
        );
      }
      if (response.status === 403) {
        throw new WhimsicalMcpError(
          "insufficient_scope",
          "Whimsical MCP denied this capability.",
          403,
        );
      }
      if (response.status === 429) {
        throw new WhimsicalMcpError(
          ...
          "Whimsical MCP rate limit reached.",
          429,
        );
      }
      if (!response.ok && response.status !== 202 && response.status !== 204) {
        throw new WhimsicalMcpError(
          ...
          `Whimsical MCP returned HTTP ${response.status}.`,
          response.status,
        );
      }
      return response;
    } catch (error) {
      if (error instanceof WhimsicalMcpError) throw error;
      throw new WhimsicalMcpError(
        "provider_unavailable",
        "Whimsical MCP could not be reached.",
        502,
      );
    }
  }
  ...
  private rejectCredentialFields(value: unknown, depth = 0) {
    if (depth > 12) {
      throw new WhimsicalMcpError(
        "policy_blocked",
        "Whimsical MCP arguments are too deeply nested.",
        403,
      );
    }
