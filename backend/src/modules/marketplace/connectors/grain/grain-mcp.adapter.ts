...
export const GRAIN_WRITE_TOOLS = [
  "create_clip",
  "tag_meetings",
  "create_collection",
...
export class GrainMcpError extends Error {
  constructor(
...
export class GrainMcpAdapter {
  async health(accessToken: string) {
    return this.withSession(accessToken, async (session) => {
      ...
      for (const required of [...GRAIN_READ_TOOLS, ...GRAIN_WRITE_TOOLS]) {
        if (!tools.some((tool) => tool.name === required)) {
          throw new GrainMcpError(
            "provider_validation_error",
            `Grain MCP did not expose the documented ${required} tool with a valid object schema.`,
          );
        }
      }
      return {
        ...
        readToolCount: GRAIN_READ_TOOLS.length,
        writeToolCount: GRAIN_WRITE_TOOLS.length,
      };
    });
  }

  callRead(accessToken: string, input: JsonObject) {
    return this.callAllowed(accessToken, input, new Set<string>(GRAIN_READ_TOOLS));
  }

  callWrite(accessToken: string, input: JsonObject) {
    return this.callAllowed(accessToken, input, new Set<string>(GRAIN_WRITE_TOOLS));
  }
  ...
  private async callAllowed(accessToken: string, input: JsonObject, allowed: Set<string>) {
    ...
    if (!allowed.has(toolName)) {
      throw new GrainMcpError(
        "policy_blocked",
        `Grain MCP tool ${toolName} is not allowed by this Relay action.`,
        403,
      );
    }
    ...
    if (Buffer.byteLength(JSON.stringify(args)) > 1_000_000) {
      throw new GrainMcpError("provider_validation_error", "Grain MCP arguments exceed 1 MB.");
    }
    return this.withSession(accessToken, async (session) => {
      ...
      if (!tool) {
        throw new GrainMcpError(
          "provider_validation_error",
          `Grain MCP did not expose ${toolName} with a valid object schema.`,
        );
      }
      ...
      if (result.isError === true) {
        throw new GrainMcpError("provider_validation_error", `Grain MCP ${toolName} failed.`);
      }
  ...
  private async withSession<T>(accessToken: string, fn: (session: Session) => Promise<T>) {
    if (!accessToken) {
      throw new GrainMcpError("credential_missing", "Grain OAuth access token is required.", 401);
    }
    ...
    if (!this.object(initialized.capabilities).tools) {
      throw new GrainMcpError("provider_validation_error", "Grain MCP did not advertise tool support.");
    }
  ...
  private async listTools(accessToken: string, session: Session): Promise<McpTool[]> {
    ...
    for (let page = 0; page < 4; page += 1) {
      ...
      for (const item of Array.isArray(result.tools) ? result.tools : []) {
        ...
        if (!name || inputSchema.type !== "object" || tools.some((candidate) => candidate.name === name)) {
          continue;
        }
  ...
  private async rpc(accessToken: string, session: Session, method: string, params: JsonObject) {
    ...
    if (Buffer.byteLength(raw) > 5_000_000) {
      throw new GrainMcpError("provider_validation_error", "Grain MCP response exceeds 5 MB.");
    }
    const payloads = this.payloads(raw);
    const payload = payloads.find((item) => item.id === id) ?? payloads[0];
    if (!payload) {
      throw new GrainMcpError("provider_unavailable", "Grain MCP returned an empty response.", 502);
    }
    if (payload.error) {
      throw new GrainMcpError(
        "provider_validation_error",
        this.string(this.object(payload.error).message) ?? "Grain MCP request failed.",
      );
    }
  ...
  private async request(accessToken: string, session: Session, body: JsonObject) {
    try {
      const response = await safeConnectorFetch("https://api.grain.com/_/mcp", {
        method: "POST",
        headers: {
      ...
      if (response.status === 401) {
        throw new GrainMcpError("credential_missing", "Grain MCP rejected the OAuth token.", 401);
      }
      if (response.status === 403) {
        throw new GrainMcpError("insufficient_scope", "Grain MCP denied this capability.", 403);
      }
      if (response.status === 429) {
        throw new GrainMcpError("provider_rate_limited", "Grain MCP rate limit reached.", 429);
      }
      if (!response.ok && response.status !== 202 && response.status !== 204) {
        throw new GrainMcpError(
          response.status >= 500 ? "provider_unavailable" : "provider_validation_error",
          `Grain MCP returned HTTP ${response.status}.`,
          response.status,
        );
      }
      return response;
    } catch (error) {
      if (error instanceof GrainMcpError) throw error;
      throw new GrainMcpError("provider_unavailable", "Grain MCP could not be reached.", 502);
    }
  }
  ...
  private rejectCredentialFields(value: unknown, depth = 0) {
    if (depth > 12) {
      throw new GrainMcpError("policy_blocked", "Grain MCP arguments are too deeply nested.", 403);
    }
