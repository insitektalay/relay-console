...
export const LUCIDSPARK_READ_OPERATIONS: readonly Operation[] = [
...
export const LUCIDSPARK_WRITE_OPERATIONS: readonly Operation[] = [
...
export class LucidsparkApiError extends Error {
  constructor(
...
@Injectable()
export class LucidsparkApiAdapter {
  ...
  async callRead(accessToken: string, input: JsonObject) {
    ...
    }, LUCIDSPARK_READ_OPERATIONS);
  }

  async callWrite(accessToken: string, input: JsonObject) {
    const path = this.requiredPath(input.path);
    let json = input.json === undefined ? undefined : this.object(input.json);
    if (path === "/v1/documents") (json ??= {}).product = "lucidspark";
    return await this.request(accessToken, {
  ...
  private async request(
    ...
    if (!accessToken?.trim() || accessToken.length > 10_000) {
      throw new LucidsparkApiError("credential_missing", "Lucidspark OAuth access token is required.", 401);
    }
    if (!allowed.some((operation) => operation.method === input.method && this.matches(operation.path, input.path))) {
      throw new LucidsparkApiError("provider_validation_error", "Lucidspark method or path is outside the supported Lucid REST boundary.");
    }
    this.rejectCredentials(input.query);
    this.rejectCredentials(input.json);
    await this.assertLucidsparkContext(accessToken, input);
    ...
    if (body && Buffer.byteLength(body) > 4_000_000) {
      throw new LucidsparkApiError("provider_validation_error", "Lucidspark request exceeds 4 MB.");
    }
    try {
      ...
      if (raw.length > 16_000_000) throw new LucidsparkApiError("provider_validation_error", "Lucidspark response exceeds 16 MB.");
      ...
      if (!response.ok) {
        throw new LucidsparkApiError(this.safeCode(response.status), this.message(data) ?? `Lucid returned HTTP ${response.status}.`, response.status);
      }
      ...
      return this.filterLucidspark(data);
    } catch (error) {
      if (error instanceof LucidsparkApiError) throw error;
      throw new LucidsparkApiError("provider_unavailable", "Lucid could not be reached.", 502);
    }
  }

  private async assertLucidsparkContext(
    accessToken: string,
    input: { method: Operation["method"]; path: string; json?: JsonObject },
  ) {
    if (input.path === "/v1/documents") {
      if (input.json?.product !== "lucidspark") throw new LucidsparkApiError("provider_validation_error", "Lucidspark creation must use the lucidspark product.");
      return;
    }
    if (input.path === "/v1/documents/search") {
      if (!Array.isArray(input.json?.product) || input.json?.product.length !== 1 || input.json.product[0] !== "lucidspark") {
        throw new LucidsparkApiError("provider_validation_error", "Lucidspark search must be restricted to Lucidspark boards.");
      }
      return;
    }
    if (input.path === "/v1/documents/copy") {
      const template = typeof input.json?.template === "string" ? input.json.template.trim() : "";
      if (!template) throw new LucidsparkApiError("provider_validation_error", "Lucidspark copy requires a template board ID.");
  ...
  private async assertDocument(accessToken: string, id: string) {
    if (!/^[A-Za-z0-9._:@%+=~-]{1,300}$/.test(id)) throw new LucidsparkApiError("provider_validation_error", "Lucidspark board ID is invalid.");
    ...
    const raw = Buffer.from(await response.arrayBuffer());
    if (raw.length > 2_000_000) throw new LucidsparkApiError("provider_validation_error", "Lucid validation response exceeds 2 MB.");
    const data = raw.length ? JSON.parse(raw.toString("utf8")) as unknown : null;
    if (!response.ok) throw new LucidsparkApiError(this.safeCode(response.status), this.message(data) ?? `Lucid returned HTTP ${response.status}.`, response.status);
    this.assertProduct(data);
  }

  private assertProduct(value: unknown) {
    if (!value || typeof value !== "object" || Array.isArray(value) || (value as JsonObject).product !== "lucidspark") {
      throw new LucidsparkApiError("provider_validation_error", "The requested Lucid document is not a Lucidspark board.", 422);
    }
  }

  private filterLucidspark(value: unknown, depth = 0): unknown {
    if (depth > 20) return "[truncated]";
    if (Array.isArray(value)) return value.map((entry) => this.filterLucidspark(entry, depth + 1)).filter((entry) => entry !== undefined);
    if (!value || typeof value !== "object") return value;
    const object = value as JsonObject;
    if (typeof object.product === "string" && object.product !== "lucidspark") return undefined;
    return Object.fromEntries(Object.entries(object).map(([key, item]) => [key, this.filterLucidspark(item, depth + 1)]));
  }
  ...
  private requiredPath(value: unknown) {
    if (typeof value !== "string" || !value.startsWith("/v1/") || value.includes("..") || value.includes("//") || value.includes("?") || value.includes("#") || value.length > 2_000) {
      throw new LucidsparkApiError("provider_validation_error", "Lucidspark path is invalid.");
    }
    return value;
  }
  ...
  private readMethod(value: unknown): "GET" | "POST" {
    ...
    if (method !== "GET" && method !== "POST") throw new LucidsparkApiError("provider_validation_error", "Lucidspark read method must be GET or POST.");
    return method;
  }
  ...
  private writeMethod(value: unknown): "POST" | "PUT" | "PATCH" | "DELETE" {
    ...
    if (method !== "POST" && method !== "PUT" && method !== "PATCH" && method !== "DELETE") {
      throw new LucidsparkApiError("provider_validation_error", "Lucidspark write method must be POST, PUT, PATCH, or DELETE.");
    }
    return method;
  }
  ...
  private accept(value: unknown) {
    ...
    if (!["application/json", "image/png", "image/jpeg"].includes(accept)) throw new LucidsparkApiError("provider_validation_error", "Lucidspark response type is not supported.");
    return accept;
  }

  private appendQuery(params: URLSearchParams, value: JsonObject) {
    if (Object.keys(value).length > 50) throw new LucidsparkApiError("provider_validation_error", "Lucidspark request has too many query fields.");
    for (const [key, item] of Object.entries(value)) {
      if (item === undefined || item === null || item === "") continue;
      if (!/^[A-Za-z0-9_.\[\]-]{1,100}$/.test(key)) throw new LucidsparkApiError("provider_validation_error", "Lucidspark query key is invalid.");
      for (const entry of Array.isArray(item) ? item.slice(0, 100) : [item]) {
        if (!["string", "number", "boolean"].includes(typeof entry)) throw new LucidsparkApiError("provider_validation_error", `Lucidspark query field ${key} must be scalar.`);
  ...
  private rejectCredentials(value: unknown, depth = 0) {
    ...
    if (depth > 12) throw new LucidsparkApiError("policy_blocked", "Lucidspark request is too deeply nested.", 403);
    for (const [key, item] of Object.entries(value as JsonObject)) {
      if (/(authorization|access.?token|refresh.?token|client.?secret|api.?key|password|cookie|credential)/i.test(key)) throw new LucidsparkApiError("policy_blocked", `Credential-bearing field ${key} is not allowed.`, 403);
