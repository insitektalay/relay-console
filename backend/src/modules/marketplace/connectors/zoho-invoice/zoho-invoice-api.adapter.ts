...
export type ZohoInvoiceCredentials = {
  accessToken: string;
  apiOrigin: string;
...
@Injectable()
export class ZohoInvoiceApiAdapter {
  constructor(private readonly request: HttpClient = fetch) {}
  async health(credentials: ZohoInvoiceCredentials) {
  ...
  async getOrganization(credentials: ZohoInvoiceCredentials) {
    ...
    if (!exact)
      throw new ZohoInvoiceApiError(
        ...
        "Zoho Invoice did not return the exact consent-bound organization.",
      );
    return { organization: this.organization(exact), nextPageFollowed: false };
  }
  private async get(
    credentials: ReturnType<ZohoInvoiceApiAdapter["credentials"]>,
  ) {
    let response: Response;
    try {
      response = await this.request(
        `${credentials.apiOrigin}/invoice/v3/organizations`,
        {
          method: "GET",
          headers: {
    ...
    if (Buffer.byteLength(raw) > 1_000_000)
      throw new ZohoInvoiceApiError(
        "provider_validation_error",
        "Zoho Invoice response exceeded the safe size limit.",
      );
    if (!response.ok)
      throw new ZohoInvoiceApiError(
        response.status === 401
          ...
          : response.status === 403
            ...
            : response.status === 429
              ...
              : response.status >= 500
                ? "provider_unavailable"
                : "provider_validation_error",
        "Zoho Invoice API request failed.",
        response.status,
      );
    try {
      ...
      throw new ZohoInvoiceApiError(
        "provider_validation_error",
        "Zoho Invoice returned an invalid response.",
      );
    }
  }
  private credentials(credentials: ZohoInvoiceCredentials) {
    ...
    try {
      ...
      throw new ZohoInvoiceApiError(
        "provider_validation_error",
        "Zoho Invoice regional API origin is invalid.",
      );
    }
    if (!accessToken || accessToken.length > 16_384)
      throw new ZohoInvoiceApiError(
        "credential_missing",
        "Zoho Invoice OAuth credentials are missing or invalid.",
      );
    if (!/^[1-9][0-9]{0,19}$/.test(organizationId))
      throw new ZohoInvoiceApiError(
        "provider_validation_error",
        "A valid exact Zoho Invoice organization binding is required.",
      );
