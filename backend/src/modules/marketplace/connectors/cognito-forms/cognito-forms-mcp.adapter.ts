...
export const COGNITO_FORMS_MCP_URL = "https://mcp.cognitoforms.com/mcp";
export const COGNITO_FORMS_MCP_RESOURCE = "https://mcp.cognitoforms.com";
export const COGNITO_FORMS_MCP_CLIENT_ID =
  "366c9904-def2-451a-af2f-8e08d751088c";

export const COGNITO_FORMS_READ_TOOLS = [
  "get_entry",
  "get_form_entries",
  "get_document",
  "get_file",
...
export const COGNITO_FORMS_WRITE_TOOLS = [
  "generate_form",
  "create_entry",
  "update_entry",
  "delete_entry",
...
export class CognitoFormsMcpError extends Error {
  constructor(
...
@Injectable()
export class CognitoFormsMcpAdapter {
    ...
    return this.withSession(accessToken, async (session) => {
      ...
      for (const required of [
        ...
        if (!tools.some((tool) => tool.name === required)) {
          throw new CognitoFormsMcpError(
            "provider_validation_error",
            `Cognito Forms MCP did not expose the documented ${required} tool with a valid object schema.`,
          );
        }
      }
      return {
        ...
        readToolCount: COGNITO_FORMS_READ_TOOLS.length,
        writeToolCount: COGNITO_FORMS_WRITE_TOOLS.length,
      };
    });
  }
  ...
  callRead(accessToken: string, input: JsonObject) {
    return this.callAllowed(
      accessToken,
      input,
      new Set<string>(COGNITO_FORMS_READ_TOOLS),
    );
  }
  ...
  callWrite(accessToken: string, input: JsonObject) {
    return this.callAllowed(
      accessToken,
      input,
      new Set<string>(COGNITO_FORMS_WRITE_TOOLS),
    );
  }
  ...
  private async callAllowed(
    ...
    if (!allowed.has(toolName)) {
      throw new CognitoFormsMcpError(
        "policy_blocked",
        `Cognito Forms MCP tool ${toolName} is not allowed by this Relay action.`,
        403,
      );
    }
    ...
    if (Buffer.byteLength(JSON.stringify(args)) > 1_000_000) {
      throw new CognitoFormsMcpError(
        "provider_validation_error",
        "Cognito Forms MCP arguments exceed 1 MB.",
      );
    }
    return this.withSession(accessToken, async (session) => {
      ...
      if (!tool) {
        throw new CognitoFormsMcpError(
          "provider_validation_error",
          `Cognito Forms MCP did not expose ${toolName} with a valid object schema.`,
        );
      }
      ...
      if (result.isError === true) {
        throw new CognitoFormsMcpError(
          "provider_validation_error",
          `Cognito Forms MCP ${toolName} failed.`,
        );
      }
  ...
  private async withSession<T>(
    ...
    if (!accessToken) {
      throw new CognitoFormsMcpError(
        "credential_missing",
        "Cognito Forms OAuth access token is required.",
        401,
      );
    }
    ...
    if (!this.object(initialized.capabilities).tools) {
      throw new CognitoFormsMcpError(
        "provider_validation_error",
        "Cognito Forms MCP did not advertise tool support.",
      );
    }
  ...
  private async rpc(
    ...
    if (Buffer.byteLength(raw) > 5_000_000) {
      throw new CognitoFormsMcpError(
        "provider_validation_error",
        "Cognito Forms MCP response exceeds 5 MB.",
      );
    }
    ...
    if (!payload) {
      throw new CognitoFormsMcpError(
        ...
        "Cognito Forms MCP returned an empty response.",
        502,
      );
    }
    if (payload.error) {
      throw new CognitoFormsMcpError(
        ...
        this.string(this.object(payload.error).message) ??
          "Cognito Forms MCP request failed.",
      );
    }
  ...
  private async request(
    ...
    try {
      const response = await safeConnectorFetch(COGNITO_FORMS_MCP_URL, {
        method: "POST",
        headers: {
      ...
      if (response.status === 401) {
        throw new CognitoFormsMcpError(
          "credential_missing",
          "Cognito Forms MCP rejected the OAuth token.",
          401,
        );
      }
      if (response.status === 403) {
        throw new CognitoFormsMcpError(
          "insufficient_scope",
          "Cognito Forms MCP denied this capability.",
          403,
        );
      }
      if (response.status === 429) {
        throw new CognitoFormsMcpError(
          ...
          "Cognito Forms MCP rate limit reached.",
          429,
        );
      }
      if (!response.ok && response.status !== 202 && response.status !== 204) {
        throw new CognitoFormsMcpError(
          response.status >= 500
            ? "provider_unavailable"
            : "provider_validation_error",
          `Cognito Forms MCP returned HTTP ${response.status}.`,
          response.status,
        );
      }
      return response;
    } catch (error) {
      if (error instanceof CognitoFormsMcpError) throw error;
      throw new CognitoFormsMcpError(
        "provider_unavailable",
        "Cognito Forms MCP could not be reached.",
        502,
      );
    }
  }
  ...
  private rejectCredentialFields(value: unknown, depth = 0) {
    if (depth > 12) {
      throw new CognitoFormsMcpError(
        "policy_blocked",
        "Cognito Forms MCP arguments are too deeply nested.",
        403,
      );
    }
