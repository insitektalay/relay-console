...
export const XMIND_READ_TOOLS = [
  "xmind_list_mindmaps",
  "xmind_read_mindmap",
] as const;
export const XMIND_WRITE_TOOLS = [
  "xmind_create_mindmap",
  "xmind_edit_mindmap",
] as const;

export class XMindMcpError extends Error {
  constructor(
...
export class XMindMcpAdapter {
  async health(accessToken: string) {
    return this.withSession(accessToken, async (session) => {
      ...
      for (const required of [...XMIND_READ_TOOLS, ...XMIND_WRITE_TOOLS]) {
        if (!tools.some((tool) => tool.name === required)) {
          throw new XMindMcpError(
            "provider_validation_error",
            `XMind MCP did not expose the documented ${required} tool with a valid object schema.`,
          );
        }
      }
      return {
        ...
        readToolCount: XMIND_READ_TOOLS.length,
        writeToolCount: XMIND_WRITE_TOOLS.length,
      };
    });
  }
  ...
  callRead(accessToken: string, input: JsonObject) {
    return this.callAllowed(
      accessToken,
      input,
      new Set<string>(XMIND_READ_TOOLS),
    );
  }
  ...
  callWrite(accessToken: string, input: JsonObject) {
    return this.callAllowed(
      accessToken,
      input,
      new Set<string>(XMIND_WRITE_TOOLS),
    );
  }
  ...
  private async callAllowed(
    ...
    if (!allowed.has(toolName)) {
      throw new XMindMcpError(
        "policy_blocked",
        `XMind MCP tool ${toolName} is not allowed by this Relay action.`,
        403,
      );
    }
    ...
    if (Buffer.byteLength(JSON.stringify(args)) > 1_000_000) {
      throw new XMindMcpError(
        "provider_validation_error",
        "XMind MCP arguments exceed 1 MB.",
      );
    }
    return this.withSession(accessToken, async (session) => {
      ...
      if (!tool) {
        throw new XMindMcpError(
          "provider_validation_error",
          `XMind MCP did not expose ${toolName} with a valid object schema.`,
        );
      }
  ...
  private async withSession<T>(
    ...
    if (!accessToken)
      throw new XMindMcpError(
        "credential_missing",
        "XMind OAuth access token is required.",
        401,
      );
    ...
    if (!this.object(initialized.capabilities).tools) {
      throw new XMindMcpError(
        "provider_validation_error",
        "XMind MCP did not advertise tool support.",
      );
    }
  ...
  private async rpc(
    ...
    if (Buffer.byteLength(raw) > 5_000_000)
      throw new XMindMcpError(
        "provider_validation_error",
        "XMind MCP response exceeds 5 MB.",
      );
    ...
    if (!payload)
      throw new XMindMcpError(
        ...
        "XMind MCP returned an empty response.",
        502,
      );
    if (payload.error)
      throw new XMindMcpError(
  ...
  private async request(
    ...
    try {
      const response = await safeConnectorFetch("https://app.xmind.com/api/mcp", {
        method: "POST",
        headers: {
      ...
      if (response.status === 401)
        throw new XMindMcpError(
          "credential_missing",
          "XMind MCP rejected the OAuth token.",
          401,
        );
      if (response.status === 403)
        throw new XMindMcpError(
          "insufficient_scope",
          "XMind MCP denied this capability.",
          403,
        );
      if (response.status === 429)
        throw new XMindMcpError(
          "provider_rate_limited",
          "XMind MCP rate limit reached.",
          429,
        );
      if (!response.ok && response.status !== 202 && response.status !== 204) {
        throw new XMindMcpError(
          response.status >= 500
            ? "provider_unavailable"
            : "provider_validation_error",
          `XMind MCP returned HTTP ${response.status}.`,
          response.status,
        );
      }
      return response;
    } catch (error) {
      if (error instanceof XMindMcpError) throw error;
      throw new XMindMcpError(
        "provider_unavailable",
        "XMind MCP could not be reached.",
        502,
      );
    }
  }
  ...
  private rejectCredentialFields(value: unknown, depth = 0) {
    if (depth > 12)
      throw new XMindMcpError(
        "policy_blocked",
        "XMind MCP arguments are too deeply nested.",
        403,
      );
