...
- List subscriptions for customer ID `1` and summarize status, renews_at, ends_at, product_id, variant_id, and update_payment_method URL presence.
- Retrieve order `104e18a2-d755-4d4b-80c4-a6c1dcbe1c10` and summarize currency, total, product, variant, and license key short values.
- Check products and variants that have files attached and identify disabled variants.

## Common Draft Tasks

- Draft only: Create checkout links or custom-priced checkout sessions.
- Draft only: Refund orders or subscription invoices.
- Draft only: Update/cancel/resume/pause subscriptions or record usage.
- Draft only: Create/delete discounts, update variants/files/download availability, modify license keys or license activations, or create/update/delete webhooks.
