...
- Retrieve customer `acme-us-001` with subscriptions and summarize invoice balance without exposing payment source details.
- Create an estimate for adding item price `pro-monthly-usd` to subscription `sub_123` and report proration before asking approval.
- List recent events for invoice `inv_123` and identify whether a webhook delivered payment_succeeded.

## Common Draft Tasks

- Draft only: Create/send/collect invoices or create hosted checkout/payment pages.
- Draft only: Refund payments, issue credit notes, write off/void invoices, or apply credits.
- Draft only: Create/update/cancel/pause/resume/reactivate subscriptions or change item prices/plans/coupons.
- Draft only: Create/update webhook settings, export customer billing data, or change entitlement-affecting subscription items.
