...
const allowed = [
  action("read_accessible", "Read accessible provider resources", "Read Discord resources visible to the connected account and summarize state."),
...
export const DISCORD_APPROVAL_PROFILES = [
  { id: "discord_read_only", label: "Read Only", description: "Read-only Discord operation for audits, summaries and safe discovery.", defaultSelected: false, allowedActions: allowed.filter((item) => item.id === "read_accessible"), approvalRequiredActions: approvalRequired, blockedActions },
  { id: "discord_safe_operator", label: "Safe Operator", description: "Default Discord operator. Reads, summaries and drafts are allowed; side effects are approval-gated.", defaultSelected: true, allowedActions: allowed, approvalRequiredActions: approvalRequired, blockedActions },
  { id: "discord_manager_approval", label: "Manager Approval", description: "Allows approved Discord writes after explicit review, target confirmation and audit logging.", defaultSelected: false, allowedActions: allowed, approvalRequiredActions: approvalRequired, blockedActions },
  { id: "discord_admin_high_risk", label: "Admin High Risk", description: "Administrative Discord profile. Destructive, secret-exposure, ownership and security-reducing actions remain blocked.", defaultSelected: false, allowedActions: allowed, approvalRequiredActions: approvalRequired, blockedActions },
];
