...
const allowed = [
  action("read_accessible", "Read accessible provider resources", "Read Canva resources visible to the connected account and summarize state."),
...
export const CANVA_APPROVAL_PROFILES = [
  { id: "canva_read_only", label: "Read Only", description: "Read-only Canva operation for audits, summaries and safe discovery.", defaultSelected: false, allowedActions: allowed.filter((item) => item.id === "read_accessible"), approvalRequiredActions: approvalRequired, blockedActions },
  { id: "canva_safe_operator", label: "Safe Operator", description: "Default Canva operator. Reads, summaries and drafts are allowed; side effects are approval-gated.", defaultSelected: true, allowedActions: allowed, approvalRequiredActions: approvalRequired, blockedActions },
  { id: "canva_manager_approval", label: "Manager Approval", description: "Allows approved Canva writes after explicit review, target confirmation and audit logging.", defaultSelected: false, allowedActions: allowed, approvalRequiredActions: approvalRequired, blockedActions },
  { id: "canva_admin_high_risk", label: "Admin High Risk", description: "Administrative Canva profile. Destructive, secret-exposure, ownership and security-reducing actions remain blocked.", defaultSelected: false, allowedActions: allowed, approvalRequiredActions: approvalRequired, blockedActions },
];
